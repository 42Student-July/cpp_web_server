...
void split_test(){
	...
	std::string ee = "////a!	a!a/b!b!b//////	////// //!!!!!!!!!!!!!//////c	 c!cc/!/!!/!!!!";
...
void print_location_context(std::map<std::string, LocationContext> l){
	size_t i = 0;
	for(std::map<std::string , LocationContext>::iterator it = l.begin();it != l.end(); it++, i++){
		std::cout <<"======location num: " << i + 1 << " ================"<< std::endl;
...
void parser_err_case(){
	...
	parser_run(path + "auto_index_invalid_argment","auto_index_invalid_argment", 1);
	std::cout <<"===========index err=============" <<std::endl;
	parser_run(path + "index_no_val","index_no_val", 1);
	std::cout <<"===========cgi_extension err=============" <<std::endl;
	parser_run(path + "cgi_extension_no_extension","cgi_extension_no_extension", 1);
	parser_run(path + "cgi_extension_no_comma","cgi_extension_no_comma", 1);
	parser_run(path + "cgi_extension_middle_comma","cgi_extension_middle_comma", 1);
