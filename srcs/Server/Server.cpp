...
std::string Server::ReplaceURILocation(
  ...
  if (last_slash != std::string::npos) {
    std::string file_path = request_path.substr(0, last_slash + 1);
    std::string file_name = request_path.substr(last_slash + 1);
    for (std::map<std::string, LocationContext>::const_iterator itr =
      ...
      if (file_path.size() >= path.size() &&
          std::equal(path.begin(), path.end(), file_path.begin())) {
        return (root + "/" + file_path.erase(0, path.size()) + file_name);
      }
    }
  }
