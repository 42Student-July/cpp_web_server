...
void ListenFd::GenerateConnectableFd(){
	addrinfo *current;
	for(current = list_top_; current; current = current->ai_next){
	...
	if(!current)
		cout << "err1"<< endl;
	if(listen(listen_.GetFd(), listen_max) < 0)
		cout << "err2"<< endl;
}

int ListenFd::MakeSocket(addrinfo* addr) const{
	return socket(addr->ai_family, addr->ai_socktype, addr->ai_protocol);
}

void ListenFd::SetSocketOption()const{
	int optval = 1;
	setsockopt(listen_.GetFd(), SOL_SOCKET, SO_REUSEADDR, (const void *)&optval, sizeof(int));
}

bool ListenFd::CanBind(addrinfo *addr)const{
	if(bind(listen_.GetFd(), addr->ai_addr, addr->ai_addrlen) == 0)
		return true;
	return false;
}
int ListenFd::GetFd()const{
	return listen_.GetFd();
}
int	ListenFd::AcceptFd()const{
	sockaddr_storage client_addr;
	char hostname[max_line], client_port[max_line];
	socklen_t client_len = sizeof(sockaddr_storage);
	int connfd = accept(listen_.GetFd(), (SA *)&client_addr, &client_len);
	getnameinfo((SA *)& client_addr, client_len,hostname, max_line, client_port, max_line, 0);
	cout << "accepted connection from (" << hostname << ", " << client_port << ")" << endl;
	return connfd;
}

bool ListenFd::IsConnecting(const epoll_event &event)const{
	return event.data.fd == GetFd();
}